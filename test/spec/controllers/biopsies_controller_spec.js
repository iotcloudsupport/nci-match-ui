describe('Controller: Biopsies Controller', function () {

    beforeEach(module('config.matchbox', 'http.matchbox', 'biopsies.matchbox'));
    ...
    it('should populate the biopsies list with 1 biopsy on a success response', function() {
        ...
        scope.loadBiopsiesList();
        httpBackend.flush();

        expect(scope.biopsiesList.length).toBe(1);
        expect(scope.biopsiesList[0].patientSequenceNumber).toBe('10368');
        expect(scope.biopsiesList[0].biopsySeqenuceNumber).toBe('N-15-00005');
        expect(scope.biopsiesList[0].molecularSequenceNumber).toBe('10368_1000_N-15-00005');
        expect(scope.biopsiesList[0].lab).toBe('Boston');
        expect(scope.biopsiesList[0].trackingNumber).toBe('987654321');
    });
    ...
    it('should not populate the biopsies list on an error response', function() {
        ...
        expect(scope.biopsiesList.length).toBe(0);
    });
